  ...
  fetch(BREEDS_URL)
      ...
      .then(data => {
        ...
        for( let i = 0; i<breedsArray.length; i++ ){
          //create <option> for each array element
          const option = document.createElement('option');
          // <option.value = 'german'>
          option.value = breedsArray[i]
          //<option value ='german'>german</option>
          option.innerText = breedsArray[i];
          //adds current <option> to the select box list of options
          select.appendChild(option); //const select see above
        }
      })
      .catch(err => {
...
const doggoInfo = {
  ...
  assignName(arr){
    ...
    document.querySelector('#dog-name').innerText = `${this.rname}`
  },

  assignAge(){
    this.age = Math.floor(Math.random() * 16+2);
    document.querySelector('#age').innerText = `Age: ${this.age}`;
  },

  yatesShuffle(arr){
    let m = arr.length, t, i;
    //While tere remain elements to shuffle...
    while( m ){
  ...
  assignLikes(){
    ...
    document.querySelector("#likes").innerText = `Likes: ${this.likes[0]}, ${this.likes[1]}`;
  },

  assignDislikes(){
    this.dislikes = this.yatesShuffle(this.dislikesList).slice(0, 2);
    document.querySelector("#dislikes").innerText = `Dislikes: ${this.dislikes[0]}, ${this.dislikes[1]}`;
  },

  assignFunFact(){
    this.fact = this.factList[Math.floor(Math.random()*this.factList.length)];
    document.querySelector('#fun-fact').innerText = `Additional Info: ${this.fact}`;
  }
}